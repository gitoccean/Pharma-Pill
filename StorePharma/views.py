...
from Authentication.models import db_Profile
...
# # ***********************************   *Medicine*   ***************************************************
class medicine(TemplateView):
    template_name = 'User/Medicine.html'
...
def add_Medicine(request):
    ...
    try:
        obj_Medicine = db_Medicine.objects.create(Medicine_ID=Medicine_ID, Med_LabelName=var_Med_LabelName,
                                                  Med_CompanyName=var_Med_CompanyName,
                                                  Med_FormulaName=var_Med_FormulaName,
                                                  Med_Purchase_Price=var_Med_Purchase_Price,
                                                  Med_Sell_Price=var_Med_Sell_Price, Med_Type=var_Med_Type,
                                                  Med_UsedFor=var_Med_UsedFor, Med_Describtion=var_Med_Describtion,
                                                  Med_ProductionDate=var_Med_ProductionDate,
                                                  Med_photo=var_img_display, Med_ExpireDate=var_Med_ExpireDate,
                                                  Med_Status=var_Status, Med_Size=var_Med_Size,  # ml, mg ,mm , cc
                                                  Med_Quantity=var_Med_Quantity,
                                                  Med_Remain_Quantity=var_Med_Remain_Quantity
                                                  )
        obj_Medicine.save()
...
def search(request):
    ...
    obj_Option = OptionList.objects.filter().all()
    mySearch = (db_Medicine.objects.filter(Med_LabelName__contains=search) | db_Medicine.objects.filter(
        Med_FormulaName__contains=search).values() | db_Medicine.objects.filter(
        Med_UsedFor__contains=search).values() | db_Medicine.objects.filter(
        Med_Describtion__contains=search).values() | db_Medicine.objects.filter(
        Med_ProductionDate__contains=search).values() | db_Medicine.objects.filter(
        Med_ExpireDate__contains=search).values())
    timeSet = Pharma_TimeTable.objects.all()
    obj_about = about_Pharma.objects.all()
    context = {
        'timeSet': timeSet,
...
def cart_list(request):
    ...
    if request.method == "POST":
        ...
        if remain_Quantity == 0:
            ...
            try:
                add_cart = med_Cart_list.objects.create(Pk_Cart_ID=auth_token, cart_email=user_email,
                                                        med_Cart_ID=var_cart_ID,
        ...
        for med_id in med_list:
            ...
            if Quantity > obj_Medicine.Med_Remain_Quantity:
                messages.success(request,
                                 'Please Make Sure your Quantity less then from available Medicine! "' + obj_Medicine.Med_CompanyName + '"')
                return redirect('cart_list')
            else:
            ...
            try:
                if med_Order.objects.filter(order_email=user_email, Pk_order_ID=bill_ID).exists() == True:
                    ...
                    obj_OrderMedicine = med_OrderMedicine.objects.create(Pk_OrderMedicine_ID=Pk_ID,
                                                                         med_OrderMedicine_email=user_email,
                                                                         order_OrderMedicine_ID=bill_ID,
                                                                         med_OrderMedicine_ID=med_id_carted,
                                                                         Pk_Cart_List_ID=med_id)
...
def optionAdd(request):
    ...
    if request.method == "POST":
        ...
        list_obj = OptionList.objects.create(Option_ID=auth_token, company_List=var_Company_list,
                                             type_List=var_Type_list)
        list_obj.save()
        ...
        if about_Pharma.objects.filter(Pk_about_ID=var_Pk_about_ID).exists() == True:
            ...
            try:
                add_about = about_Pharma.objects.create(Pk_about_ID=auth_token,
                                                        ...
                                                        about_admin_email=var_about_admin_email,
                                                        about_address=var_about_address,
...
# dashboard 32 {{contxt_obj_User|total_Amount_count}}
# dashboard 47  {{contxt_obj_User|totalEarn_count}}
# dashboard 62  {{contxt_obj_User|totalUser_count}}
# dashboard 77  {{contxt_obj_User|ActiveUser_count}}
# dashboard 93  {{contxt_obj_User|total_item_count}}
# dashboard 108 {{contxt_Med_Order|orderCancel_count}}
# dashboard 124 {{contxt_Med_Order|orderPassed_count}}
# dashboard 139  {{contxt_Med_Order|orderPending_count}}
# dashboard 154  {{contxt_Med_Order|orderWaiting_count}}

# User/User_report 137  {{ordMed.med_OrderMedicine_ID|GetName}}
# User/User_report 139  {{ordMed.Pk_Cart_List_ID|GetQuantity}}
# User/User_report 141  {{ordMed.med_OrderMedicine_ID|GetPrice}}
# User/User_report 142  {{ordMed.Pk_Cart_List_ID|GetTotalPrice}}

# Admin/add_about 4  {% load custom_tag %}
# Admin/add_blog 4  {% load custom_tag %}
# home.html 37  {% load custom_tag %}
# User/User_report.html 45 {% load custom_tag %}
# Admin/dashboard.html 1 {% load custom_tag %}
# User/cart.html row 63  {% load custom_tag %}
