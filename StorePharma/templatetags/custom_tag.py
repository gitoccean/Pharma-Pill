...
def Cart_count(Pk_Cart, email):
    ...
    Quantity = med_Cart_list.objects.filter(cart_email=email, med_Cart_ID=ID, Pk_Cart_ID=Pk_Cart).aggregate(
        TOTAL=Sum("Med_Cart_Quantity"))['TOTAL']
...
def GetTotalPrice(value):
    ...
    Quantity = med_Cart_list.objects.filter(cart_email=email, med_Cart_ID=ID, Pk_Cart_ID=value).aggregate(
        TOTAL=Sum("Med_Cart_Quantity"))['TOTAL']
    price = db_Medicine.objects.filter(Medicine_ID=ID).aggregate(TOTAL=Sum("Med_Sell_Price"))['TOTAL']
